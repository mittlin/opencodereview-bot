...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ocr-bot ./cmd/opencodereview/
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ocr-bot-server ./bot.go
